...
export function FloatingSubmitButton({ href }: FloatingSubmitButtonProps) {
  const isSignIn = href.includes("signin");
  return (
    <Link
      href={href}
      ...
      aria-label={isSignIn ? "Sign in to post a question" : "Post a question"}
      title={isSignIn ? "Sign in to post a question" : "Post a question"}
    >
